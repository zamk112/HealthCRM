...
### Rationale
- **Bootstrap logger first.** `CreateBootstrapLogger()` gives a working logger *before* the DI container is built. This follows the two-stage initialization pattern documented by Serilog: the official guidance notes that when Serilog is initialized first, the ASP.NET Core host's configuration ([appsettings.json](../HealthCRM.Server/appsettings.json)) and dependency injection are not yet available, so the bootstrap logger is created to capture anything that happens during startup, and is then replaced by the fully-configured logger once the host has loaded. The replacement is complete — the final logger does not inherit the bootstrap logger's sinks — so the configuration is supplied again via `ReadFrom.Configuration` / `ReadFrom.Services` in the second stage.
